...
log_name=${drug}_${timestamp}_${samples}.log
...
if ! Rscript generate_bootstrap_samples.R -d "$drug" > "$log_file"; then
echo "Error: generate_bootstrap_samples.R failed. Check log file for details: $log_name"
exit 1
fi
...
do
  if ! Rscript hERG_fitting.R -d "$drug" -c "$cpu" -i "$counter" -l "$population" -t "$accuracy" >> "$log_file"; then
    echo "Error: hERG_fitting.R failed. Check log file for details: $log_name"
    exit 1
  fi
done
# if ! Rscript hERG_fitting.R -d "$drug" -c "$cpu" -i "$samples" -l "$population" -t "$accuracy" >> "$log_file"; then
# echo "Error: hERG_fitting.R failed. Check log file for details: $log_name"
# exit 1
# fi

echo "Success: Rscripts completed successfully. Log file: $log_name"